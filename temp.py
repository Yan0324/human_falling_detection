...
class FallDetector:
    ...
    def __init__(self):
        ...
        self.nose_history = deque(maxlen=30)

        # 可调参数
        self.ANGLE_THRESH = 135         # 身体倾斜角度阈值
        self.CONF_THRESH = 0.4          # 关键点置信度阈值
        self.HIP_THRESH = 0.15         # 髋部下降阈值
        self.KNEE_THRESH = 0.10        # 膝盖下降阈值
    ...
    def analyze_posture(self, keypoints, bbox):
        ...
        if len(self.history) >= 15:
            return np.mean(self.history) > 0.6  # 如果超过60%的历史记录表示跌倒，则判定为跌倒
...
def main():
    ...
    parser.add_argument('--radius', type=int, default=4, help='关键点绘制半径')
    parser.add_argument('--thickness', type=int, default=2, help='连线粗细')
    parser.add_argument('--alpha', type=float, default=0.8, help='透明度')
    args_cmd = parser.parse_args()

    # 配置参数
    args = Namespace(
        det_config='/home/yc/yan/mywork/human_falling_detection/mmdetection/configs/yolox/yolox_s_8xb8-300e_coco.py',
        ...
        pose_config='/home/yc/yan/mywork/human_falling_detection/mmpose/configs/body_2d_keypoint/rtmpose/coco/rtmpose-l_8xb256-420e_aic-coco-256x192.py',
    ...
    while cap.isOpened():
        ...
        # 每秒更新一次FPS显示
        if elapsed_time >= 1.0:
            real_fps = frame_counter / elapsed_time
            frame_counter = 0
            start_time = current_time
        
        # 在画面上显示FPS
        cv2.putText(frame_out, f"FPS: {real_fps:.2f}", (10, 30), 
        ...
        if args_cmd.output:
            if video_writer is None:
                ...
                # 使用原始FPS值（或根据需求使用 real_fps 的整数值）
                video_writer = cv2.VideoWriter(output_path, fourcc, int(fps) if fps else 30, 
